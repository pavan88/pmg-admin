...
!function($) {
  ...
  var Layer = function(element, options) {
    ...
    this.contDuration = this.options.duration * 0.5;
  ...
  Layer.prototype.calculateStartStyles = function(e) {
    ...
    // get the most long distance from click to end of layer
    var mostLongDist = (function(){
      var size;
      if(layerPos.width > layerPos.height) {
        size = (layerPos.width - _this.startStyles.left) > _this.startStyles.left
               ? (layerPos.width - _this.startStyles.left)
               : _this.startStyles.left;
      } else {
        size = (layerPos.height - _this.startStyles.top) > _this.startStyles.top
               ? (layerPos.height - _this.startStyles.top)
               : _this.startStyles.top;
      }
      size *= 2 * 1.4; // 1.5 because border-radius = 50%
      return size;
    }());
    ...
    $.extend(_this.startStyles, {
      width: mostLongDist,
      height: mostLongDist,
      marginTop: -mostLongDist / 2,
      marginLeft: -mostLongDist / 2
    });
  }
